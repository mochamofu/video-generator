...
def main(argv: list[str] | None = None) -> int:
    ...
    pe = sub.add_parser("edit", help="CapCut風タイムラインエディタを起動(スマホ対応)")
    pe.add_argument("script", help="編集する台本JSON")
    pe.add_argument("--port", type=int, default=7860)
    pe.add_argument("--tts", default="auto",
                    choices=["auto", "voicevox", "edge", "openjtalk", "none"])

    px = sub.add_parser("export", help="CapCut等の編集アプリ持ち込み用に素材を書き出し")
    px.add_argument("script", help="台本JSONのパス")
    px.add_argument("-o", "--out", default=None, help="出力先ディレクトリ (省略時 export/<台本名>)")
    px.add_argument("--tts", default="auto",
                    choices=["auto", "voicevox", "edge", "openjtalk", "none"])
    px.add_argument("--voice", default=None)
    ...
    if args.cmd == "edit":
        ...
        serve(args.script, port=args.port, tts=args.tts)
        return 0

    if args.cmd == "export":
        from .export_assets import export_assets
        out = args.out or os.path.join(
            "export", os.path.splitext(os.path.basename(args.script))[0])
        export_assets(args.script, out, tts_name=args.tts, voice=args.voice)
        return 0
